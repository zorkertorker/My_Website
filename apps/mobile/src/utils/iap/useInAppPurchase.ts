...
export async function loadOfferings(setOfferings: (o: any) => void) {
  for (let attempt = 0; attempt < RETRY_ATTEMPTS; attempt++) {
    try {
      ...
      console.warn(`Failed to load offerings (attempt ${attempt + 1}/${RETRY_ATTEMPTS}):`, error);
    }
...
export async function fetchSubscriptionStatus(setIsSubscribed: (v: boolean) => void) {
  try {
...
export function getSubscriptionsFromOfferings(offerings: any) {
  return getAvailablePackagesFromOfferings(offerings).filter(
    (pkg: any) => pkg.product.productCategory === PRODUCT_CATEGORY.SUBSCRIPTION
  );
}
...
export async function executeRestore(setIsSubscribed: (v: boolean) => void) {
  try {
...
export function useInAppPurchase() {
  const { isReady, offerings, setOfferings, setIsSubscribed, isSubscribed, setIsReady } =
