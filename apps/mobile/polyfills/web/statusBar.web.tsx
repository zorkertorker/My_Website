...
function styleToBarColor(
  ...
  let resolved: Exclude<StatusBarStyle, 'auto' | 'inverted'> | 'light' | 'dark' = style as any;

  if (style === 'auto') resolved = actual === 'light' ? 'dark' : 'light';
  else if (style === 'inverted') resolved = actual === 'light' ? 'light' : 'dark';

  return resolved === 'light' ? '#FFFFFF' : '#000000';
}

export const StatusBar = React.forwardRef<any, StatusBarProps>(function StatusBar(
  { style = 'auto', ...props },
  _ref
) {
...
export const setStatusBarHidden = (hidden: boolean, animation?: StatusBarAnimation) =>
  ExpoSB.setStatusBarHidden(hidden, animation);

export const setStatusBarBackgroundColor = (backgroundColor: string, animated?: boolean) =>
